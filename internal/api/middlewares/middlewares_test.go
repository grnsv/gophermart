...
func TestAuthenticate(t *testing.T) {
	...
	mockJWTService.EXPECT().ParseCookie(gomock.Any()).Return("12345", nil).Times(1)

	handler := Authenticate(mockLogger, mockJWTService)(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func TestAuthenticate_Unauthorized(t *testing.T) {
	...
	mockJWTService.EXPECT().ParseCookie(gomock.Any()).Return("", http.ErrNoCookie).Times(1)
	mockLogger.EXPECT().Infoln(gomock.Any()).Times(1)
